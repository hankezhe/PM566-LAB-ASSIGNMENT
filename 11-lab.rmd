...
max(cv_states$cases)
```
The variable 'data' ranges from 2020-01-021 to 2020-11-01; the number of deaths ranges from 0-33,174; the number of cases ranges from 1-957,108 through the time period.
...
```
Corrected outliers with number of new cases/deaths < 0. 
...
ggplotly(p)
```
The population density of most states were concentrated below 400. 
Overall, a positive association between population density and number of new deaths per 100k was observed. The graph also showed a negative association when the population density was from 120 to 250.
...
On May 1st, 2020, the highest CFR was observed on the east coast, with Connecticut (8.13) ranked the first and followed by New York (7.6). In California, the CFR is 4.1. 
Overall, the CFR across the nation decreased drastically from May to today, CFR equals to 6.38 in Connecticut and 6.47 in New York. It is 1.88 in California.
